...
- `trackedBody` requires a fresh associated body observation for every living target from a connected, aligned observer with a fresh tracked phone. Confidence must be ≥0.8 and uncertainty ≤0.1 m. Stable collider IDs prevent interpolation between different tracks. Stale or missing coverage pauses combat and cancels existing flight. Collider ids must be consistent across an interval or coverage is lost; hits require the shooter's own fresh sighting (both geometries) and the impact must be within 2 m of the target's own authenticated phone pose; a new collider track must first appear within 2 m of that pose. `phoneProxy` is an explicit separate rule using a 0.35 m torso-zone sphere at the phone; it is not anatomical tracking.
