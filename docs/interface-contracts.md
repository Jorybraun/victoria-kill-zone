...
Proposed under [ADR 0010](decisions/0010-quick-play-relocalized-frame-and-phone-proxy.md), not yet in force: `matches:create` gains an optional additive `combatGeometry: "trackedBody" | "phoneProxy"` (absent means `trackedBody`) and `combat:prepare` writes the match's geometry into `combatRulesJson.geometry` instead of always emitting `DEFAULT_RULES`. Quick Play would create `phoneProxy` matches whose native frame uses the relocalized alignment mode (raw world map, no reference, `frameReady` residual 0, which the worker already accepts); saved-arena matches keep the measured bundle above. Protocol, validators and worker are unchanged. Until the record is accepted, the paragraphs above remain the contract.
