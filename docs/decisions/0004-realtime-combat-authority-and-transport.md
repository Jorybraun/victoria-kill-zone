...
3. **QUIC identity is not provisioned by the app, so the TCP link stays as an automatic fallback.** Network.framework QUIC needs a TLS server identity and the package pins the host's public key on clients (`TransportIdentityProvider`); the app passes `nil`, so the QUIC handshake cannot complete on device until an ephemeral per-match identity is minted at match start and its public key is published (TXT `pk` entry or the Convex match record). TLS-PSK was evaluated as the shortcut, since the match already derives a PSK from match id + join code: Apple DTS states that in Network.framework "QUIC security only supports PKI. TLS security supports both TLS-PKI and pre-shared key (PSK)" ("Moving from Multipeer Connectivity to Network Framework", developer.apple.com/forums/thread/776069), so `sec_protocol_options_add_pre_shared_key` on `NWProtocolQUIC.Options.securityProtocolOptions` is not a path. Consequently `ArenaPeerLink` (Bonjour `_pewpew-arena._tcp`, now match-scoped by instance name `vkz-<scopeId>` and using the same `ArenaLinkBodyCodec` bodies under a length prefix) now runs a mutual PSK challenge-response (`ArenaLinkAuthenticator`, HMAC-SHA256 over role + nonces with the same match PSK) before any body is accepted, so the fallback plane has the same join-secret gate as the QUIC slot claim; the TCP payload is still unencrypted. It is **kept** and `FallbackArenaPeerLink` selects it automatically when the QUIC link has not reached `connected` within 3 s or fails first (`ArenaLinkPath` records which plane is live). Deleting the TCP link moves to the slice that solves the identity problem.
