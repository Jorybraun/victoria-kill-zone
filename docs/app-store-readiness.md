...
| 2.4 | `DEBUG TORSO FALLBACK` fire button | AGENTS.md debug-fire rule | FIXED | Relabelled "DEBUG · TORSO FALLBACK FIRE", VoiceOver label added, and hidden in Release (`#if DEBUG`). The **network path** `shots:debugFire` / `LobbyStore.debugFire()` is untouched and still reached in Release by the FIRE button's automatic fallback and by Voice Fire, so the Release path does not depend on the explicit button. Physical-device evidence is still required before removing the path (see §5). |
...
Kept deliberately: `shots:debugFire`, `LobbyStore.debugFire()`, the Release-side automatic torso fallback in `ActiveDuelView.fireShot()`, and everything under `archive/`.
