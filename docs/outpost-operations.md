...
1. **Gate (hosted `ubuntu-latest`).** It reacts to a successful `Deploy` completion on this repository's `main` when `VKZ_TESTFLIGHT_ENABLED` is `true`. `scripts/release/promotion-gate.mjs` reads current main, canonical `.github/workflows/ci.yml` push history and `.github/workflows/deploy.yml` evidence through the GitHub API. The candidate must have successful CI plus successful current-attempt backend deployment, spectator deployment and smoke steps, with nonexpired SHA-named release evidence created during that attempt. The Deploy run name records the actual checkout SHA; a chained workflow's context `head_sha` is insufficient. Manual `workflow_dispatch` follows the same checks. Missing permissions or unavailable/incomplete evidence fail closed. The workflow's `testflight` concurrency group serializes promotions without cancelling an in-flight upload.
2. **Promote (`self-hosted, macos, vkz-outpost`).** The Outpost checks out the promoted SHA and revalidates current main, CI and deployment evidence immediately before archiving. A changed main reports `skipped-stale`; missing deployment prerequisites fail before archive/upload. `scripts/release/promote-testflight.mjs` then archives and signs through `testflight-upload.sh`, polls App Store Connect with exponential backoff and a timeout, posts each state to `#pew-pew-releases`, and writes sanitized evidence.

Deployment lookup is limited to the candidate's CI era and exact run name. A newer failed or pending attempt for that candidate blocks promotion; unrelated releases do not. A partial rerun that reuses earlier-attempt jobs or artifacts cannot satisfy this gate. Run a fresh full Deploy after repairing a deployment failure. `pnpm verify` includes the pure Node release and promotion self-tests.
...
- **Nothing promotes after a merge:** confirm `VKZ_TESTFLIGHT_ENABLED` is `true` and check the candidate's Deploy result before the TestFlight gate. Repair failed deployment/smoke checks and run a fresh full Deploy if evidence is missing or a partial rerun reused older jobs. A `staleSha` skip is correct; the newest successfully deployed revision promotes instead. Manual TestFlight dispatch cannot bypass deployment.
