...
| M5 — host versus DO | SQLite room authority, full terminal ledger, map storage, epoch-sealed projection outbox and 44 actual runtime tests; indexed lookups, compact idempotency records, shared broadcast encoding; [local four-player load evidence](research/backend-evidence/runtime-load/README.md) retains failed baseline and optimized results | Identical fixtures on both candidates; actual Wi-Fi/LTE measurements, projection delivery/backlog policy, idle cost, durability/reconnect/cost evidence; authority-selection ADR |
| M6 — release evidence | Game integration: 237 workspace tests, 280 native SwiftPM tests, 74 shared Swift simulation and 79 transport tests; unsigned Debug/test and Release builds. Server slice: canonical workspace verification passes 243 tests. Registered mesh/credits, bounded retries, background suspension and lifecycle regressions | Physical frame-time/thermal/battery/memory, input/VoiceOver/permission trials; signed installation/TestFlight and reviewed canonical-SHA promotion |
