...
|---|---|---|
| M0 — feedback, cadence and UI | PR #55: 150 ms cadence, hold fire, reload, hit-only effect, pooled FX, first HUD/home redesign | Anatomical 3D mesh replacing sticks; complete calibration/combat/reconnect/result UI; on-device feedback/performance checks |
| M1 — shared frame | Same-session world-map/body-tracking provider; archive/epoch/stale-callback and residual gates; 11 new automated tests | Authenticated map transfer and independent common-scene residual measurements in app; two-phone S0 evidence and exact shared endpoints |
| M2 — one combat authority | Versioned bounded protocol; authenticated four-slot lobby; short-lived signed capabilities; legacy combat writer exclusion; native clock/socket/replica | Connect full app to authority; remote-pose identity association, loss/reconnect behavior, durable spectator projection; two/four-phone convergence |
| M3 — finite projectiles | Deterministic simulation implementation in progress | Swept moving-body collision fixtures, integration/worldlines, filmed dodge/control trials and false-verdict evidence |
| M4 — shield and slow fields | Accepted tuning in ADR 0008; engine implementation in progress | Front-face/body ordering, overlap/boundary/energy/cooldown tests; native controls/rendering; physical block/slowdown trials |
| M5 — host versus DO | One SQLite Durable Object per match implementation in progress | Same fixtures on both authority candidates; actual Wi-Fi and LTE measurements at two/four players, durability/reconnect/cost evidence; authority-selection ADR |
| M6 — release evidence | Automated verification and unsigned app compile from PR #55 | Sustained frame-time/thermal/battery/memory evidence; accessibility and permissions/lifecycle testing; signed installation/TestFlight and reviewed canonical-SHA promotion |
