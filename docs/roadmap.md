...
Current execution step: **in progress — measure and harden the combat server after the playable-game integration**. The integrated game is in draft [PR #56](https://github.com/Jorybraun/victoria-kill-zone/pull/56), following release-fix #53 and conflict-resolved [PR #55](https://github.com/Jorybraun/victoria-kill-zone/pull/55). Server performance work and the four-player spectator are separate dependent review slices. Root owns integration, shared contracts and evidence; physical M1–M4 acceptance remains open.
...
| M5 — host versus DO | SQLite room authority, full terminal ledger, map storage, epoch-sealed projection outbox and 46 actual runtime tests; indexed lookups, compact idempotency records, shared broadcast encoding and bounded per-resolution collision geometry reuse. [Local load evidence](research/backend-evidence/runtime-load/README.md) isolates simulation cost and retains all failures: cached three-minute opposing combat passes; miss lanes still cancel 120 shots | Independent client/clock and command-arrival profiling for remaining pauses; identical fixtures on both authorities; actual Wi-Fi/LTE measurements, projection delivery/backlog policy, idle cost, durability/reconnect/cost evidence; authority-selection ADR |
| M6 — release evidence | Game integration after shared fixture repair: 239 workspace tests, 281 native SwiftPM tests, 74 shared Swift simulation and 79 transport tests; unsigned Debug/test and Release builds. Server slice adds collision reuse/bounded-fallback regressions (249 workspace tests). Registered mesh/credits, bounded retries, background suspension and lifecycle regressions | Physical frame-time/thermal/battery/memory, input/VoiceOver/permission trials; signed installation/TestFlight and reviewed canonical-SHA promotion |
