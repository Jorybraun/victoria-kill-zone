...
### Benchmark-only first coverage-pause trace

`VKZ_TRACE=1 pnpm --dir services/combat-worker test:load:node` selects a separate benchmark entrypoint and configuration. Its Durable Object subclass adds one local `beginTrace` RPC, invoked after the existing 500 ms warmup and completed before measurement and firing begin. The production entrypoint, HTTP router, wire messages and runtime source are unchanged. The additional premeasurement RPC creates a small gap between arming and measurement; no clock probe or synthetic clock anchor is introduced. The four clients still share one Node event loop and pose pump, while workerd runs in a separate process.

The observer retains the latest 64 admitted inputs and eight tick entries. Input metadata distinguishes the production handler-entry timestamp, SerialQueue callback entry, admission entry, assigned tick, consumed tick and result. Queue residence ends before message parsing; parsing and validation between queue entry and admission remain separately visible. Tick records include cadence anchor/lag and the last accepted pose timestamps. After the first failed coverage predicate in the running phase, the observer records the exact interval, failed players, phone history timestamps and body-history endpoint coverage. It freezes that record after the matching pause commit resolves or rejects, preserving the original return values and promises. It uses the unchanged history predicates to explain the already-failed check; no geometry, full envelope, ticket or credential is retained.

One structured log record is capped at 32,768 UTF-8 bytes, pruning oldest inputs/ticks if needed. The pinned Wrangler harness captures logs in the Node process; the reporter extracts only the trace and arming prefixes and reports malformed records explicitly. Diagnostic annotations are retained even on scenario failure. The source manifest hashes the instrumentation and separate configuration. Wrappers add timestamp, bounded bookkeeping and callback overhead during measurement; coverage-history analysis and serialization run only after the first observed failure. This is diagnostic evidence with observer overhead, not a controlled comparison to uninstrumented latency or evidence of production timing.

[node-first-pause-30s.json](node-first-pause-30s.json) preserves the single authorized 30-second pair: both scenarios passed the unchanged acceptance assertions, with 352/136 accepted and durable shots, zero cancellations, all four ledgers and spawn/terminal identities matching, and zero unresolved bullets. Both observers were armed and reported zero malformed records. Miss lanes recorded no coverage pause. Opposing combat recorded one at the first tick after arming, before the first measured pose send: tick 47, match time 2,350 ms. It restored at tick 48 and the first client observed about 51 ms paused. Passing acceptance therefore does not mean no pause occurred.

The boundary trace shows all four latest accepted poses captured at 2,229.7–2,230.3 ms, aged 119.7–120.3 ms at the failed endpoint. Four new poses captured around 2,347 ms were already pending, but trusted arrival assigned them to tick 48. Tick 47 had four pending and zero eligible inputs, queue residence was 0 ms at the available clock resolution, and cadence lag was 55 ms. Both phone and body endpoint coverage failed. The trace contains only that first tick and four post-arm inputs. Obtaining the harness environment/DO RPC at arming may have stalled the shared Node loop and contributed to this approximately 117 ms capture gap; the report does not isolate that cost. Treat this as an arming-boundary diagnostic, not a cause established for earlier sustained cancellations or the interrupted three-minute runs. No further load attempt was made to obtain a preferred result.
