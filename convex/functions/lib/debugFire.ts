...
import type { MutationCtx } from "./server.js";
import { advancePhase, authenticatePlayer, fail, listPlayers } from "./store.js";

/**
 * Shared definition of the authoritative debug fire path.
 *
 ...
 *
 * It is registered under both frozen wire names (`shots:debugFire` in
 * `docs/interface-contracts.md`, `matches:debugFire` in the integration audit)
 * so either adapter resolves the identical authoritative behavior.
 */
export const debugFireArgs = {
  matchId: v.string(),
  playerId: v.string(),
...
export async function debugFireHandler(
  ctx: MutationCtx,
  args: {
    matchId: string;
    playerId: string;
